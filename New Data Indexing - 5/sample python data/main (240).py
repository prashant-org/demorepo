...
def define_param_algo(parser):
    ...
    parser.add_argument("--loss_name", type=str, default="sigmoid", choices=["sigmoid", "softmax"], help="loss function")
    ...
    def register_query_param(parser):
        ...
        group.add_argument("--identity_feature", type=str2bool, default="False", help="whether use identitfy feature.")
...
def run_eval(param, trainer, query, model, backend):
    ...
    batch_callback = tf.keras.callbacks.LambdaCallback(
        on_test_begin=lambda logs: model.reset_metric_states()
    )
    trainer.evaluate(
        dataset=distributed_dataset,
        model=model,
